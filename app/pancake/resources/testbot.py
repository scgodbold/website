...
from secrets import TEST_BOT
# from secrets import RAINY_DAY


class TestBot(GroupmeBot):
    """A test groupme bot"""
    def __init__(self):
    ...
    def parse(self, json):
        ...
        if not message.bot:
            ...
            if message.contains('bears'):
                self.post({'text': 'http://scgodbold.com:5000/static/imgs/pancake/bears.jpg'})
            ...
            if message.contains('harbaugh'):
                self.post({'text': 'http://scgodbold.com:5000/static/imgs/pancake/harbaugh.jpg'})
    ...
    def get(self, message):
        ...
        for f in os.listdir(app.config['SET_DIR']):
            ...
            if file_name == name[1]:
                payload = {
                    'text': 'http://scgodbold.com:5000/static/imgs/pancake/setimgs/{}'.format(f),
                }
                self.post(payload)
                return
